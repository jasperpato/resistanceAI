        ...
        if len(self.worlds) > 1 and self.rnd < 4:
            ...
            for w in self.worlds.keys():
                self.worlds[w] *= self.vote_probability(w, self.missions[-1].votes_for, \
                    vsf, vss, vrf, vrs, mission_success)
                self.worlds[w] /= voting_prob

            proposer_prob = 0  # overall probability of a proposer causing mission outcome    
            for w, wp in self.worlds.items():    
                proposer_prob += self.proposer_probability(w, proposer, psf, pss, prf, prs, mission_success) * wp
            for w in self.worlds.keys(): 
                self.worlds[w] *= self.proposer_probability(w, proposer, psf, pss, prf, prs, mission_success)
